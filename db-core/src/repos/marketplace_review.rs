...
impl<'a> MarketplaceReviewRepo<'a> {
    ...
    pub fn insert(&self, r: &MarketplaceReviewRow) -> Result<(), crate::error::DbError> {
        ...
        crate::query::execute(
            ...
            params![r.id.as_str(), r.listing_id.as_str(), r.reviewer_pubkey.as_str(), r.rating, r.text.as_str(), r.created_at],
        )?;
        Ok(())
    }
    ...
    pub fn average_for(&self, listing_id: &str) -> Result<Option<f64>, crate::error::DbError> {
        ...
        crate::query::query_first(
            &conn,
            "SELECT AVG(rating) FROM marketplace_reviews WHERE listing_id=?1",
            params![listing_id],
            |r| r.get::<f64>(0),
        )
    }
