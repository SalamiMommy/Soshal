...
#[frb(serialize)]
pub async fn render_compute_mesh_frame(
    session_id: i64,
    nodes_json: String,
    delta_time: f32,
...
mod tests {
    ...
    #[tokio::test]
    async fn render_frame_empty_nodes_blank_buffer() {
        let json = render_create_session(8, 8).unwrap();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        let id = v["session_id"].as_i64().unwrap();
        let buf = render_compute_mesh_frame(id, String::new(), 0.016)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn render_frame_draws_node_pixel() {
        let json = render_create_session(8, 8).unwrap();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        let id = v["session_id"].as_i64().unwrap();
        ...
        .to_string();
        let buf = render_compute_mesh_frame(id, nodes, 0.016).await.unwrap();
    ...
    #[tokio::test]
    async fn render_invalid_session_errors() {
        let e = render_compute_mesh_frame(999, String::new(), 0.016)
            .await
            .unwrap_err();
    ...
    #[tokio::test]
    async fn render_bad_nodes_json_ignored() {
        let json = render_create_session(4, 4).unwrap();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        let id = v["session_id"].as_i64().unwrap();
        let buf = render_compute_mesh_frame(id, "not json".to_string(), 0.016)
            .await
            .unwrap();
